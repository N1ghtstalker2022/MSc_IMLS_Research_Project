...
class VRHM48Video(Dataset):
    ...
    Args:
        ...
        split: Specifies which dataset partition should be used. Valid values
    ...
    def __init__(
        ...
        if folder_list is not None:
            ...
            if split not in ["train", "test", "small_train", "small_test", "for_test", "for_pred"]:
                raise ValueError(
            ...
            with open(self.root / f"sep_{split}list.txt", "r") as file:
                self.folder_list = [
                    fname.strip() for fname in file.readlines()
                ]
    ...
    def load_image(self, septuplet: Path, frame_number: int) -> Tensor:
        img_path = default_loader(str(septuplet / f"img{frame_number}.png"))
