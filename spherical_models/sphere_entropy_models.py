...
class SphereEntropyModel(nn.Module):
    ...
    def __init__(
        ...
        self.entropy_coder = _SphereEntropyCoder(entropy_coder)
    ...
    def __setstate__(self, state):
        self.__dict__ = state
        self.entropy_coder = _SphereEntropyCoder(self.__dict__.pop("entropy_coder"))
    ...
    def compress(self, inputs, indexes, means=None):
        ...
        if len(inputs.size()) != 3:
            raise ValueError("Invalid `inputs` size. Expected a 3-D tensor.")
    ...
    def decompress(self, strings, indexes, means=None):
        ...
        if means is not None:
            if (means.size()[0] != indexes.size()[0]) or (means.size()[2] != indexes.size()[2]):
...
class SphereEntropyBottleneck(SphereEntropyModel):
    r"""Entropy bottleneck layer for spherical images_equirectangular
    """
    def __init__(self,
                 channels,
                 *args,
    ...
    def forward(self, x, training=None):
        ...
        # Convert to (channels, num_nodes , batch) format
        ...
        outputs = outputs.transpose(2, 0).contiguous()

        likelihood = likelihood.reshape(shape)
        likelihood = likelihood.transpose(2, 0).contiguous()
    ...
    def _build_indexes(size):
        N, P, C = size
        indexes = torch.arange(C).view(1, 1, -1)
        indexes = indexes.int()
        return indexes.repeat(N, P, 1)

    def compress(self, x):
        indexes = self._build_indexes(x.size())
        medians = self._get_medians().detach().view(1, -1).expand(x.size(0), 1, -1)
        return super().compress(x, indexes, medians)

    def decompress(self, strings, Npix):
        output_size = (len(strings), Npix, self._quantized_cdf.size(0))
        indexes = self._build_indexes(output_size).to(self._quantized_cdf.device)
        medians = self._get_medians().detach().view(1, -1).expand(len(strings), 1, -1)
        return super().decompress(strings, indexes, medians)


class SphereGaussianConditional(SphereEntropyModel):
