...
def create_frame_groups():
    ...
    for frame_directory in frame_directories:
        ...
        for i in range(len(cur_frames) - group_size + 1):
            ...
            path_to_check = os.path.join(output_folder, frame_directory, f"group{i + 1}")

            # Create a folder for the group
            if os.path.exists(
                os.path.join(path_to_check)
            ):
                # List all files in the directory
                files = [f for f in os.listdir(path_to_check) if os.path.isfile(os.path.join(path_to_check, f))]

                # Check if there are exactly 7 files
                if len(files) == 7:
                    print("There are exactly 7 files in the folder.")
                    print(
                        f"Folder {path_to_check} already exists."
                    )
                    continue
                    # Your code to execute when there are exactly 7 files
                else:
                    print(f"There are {len(files)} files in the folder, not 7.")
                    # Your code to execute otherwise

            group_folder = path_to_check
        ...
        print(f"Finished processing {frame_directory}.")
